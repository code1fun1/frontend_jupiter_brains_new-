...
def log_chat_operation(operation, chat_id=None, message_id=None, details=None):
    """Helper to log chat operations"""
    log.debug(f"[CHAT-DB] {operation}" + (f" - Chat ID: {chat_id}" if chat_id else "") + (f" - Message ID: {message_id}" if message_id else ""))
    if details:
        log.debug(f"[CHAT-DB] Details: {json.dumps(details, default=str, indent=2)[:500]}")
...
class ChatForm(BaseModel):
    chat: dict
    folder_id: Optional[str] = None
    auto_select_model: Optional[bool] = False
...
class ChatTable:
    ...
    def insert_new_chat(
        ...
        log.info(f"[CHAT-DB] insert_new_chat - ENTRY")
        log.debug(f"[CHAT-DB] User ID: {user_id}")
        
        with get_db_context(db) as db:
            id = str(uuid.uuid4())
            log.info(f"[CHAT-DB] Generated new chat ID: {id}")
            
            title = form_data.chat.get("title") if "title" in form_data.chat else "New Chat"
            log.debug(f"[CHAT-DB] Chat title: {title}")
            ...
            log.info(f"[CHAT-DB] Chat created successfully - ID: {chat_item.id}")
            log.debug(f"[CHAT-DB] Created at: {chat_item.created_at}, Updated at: {chat_item.updated_at}")
    ...
    def upsert_message_to_chat_by_id_and_message_id(
        ...
        log.info(f"[CHAT-DB] upsert_message_to_chat_by_id_and_message_id - ENTRY")
        log.debug(f"[CHAT-DB] Chat ID: {id}, Message ID: {message_id}")
        
        chat = self.get_chat_by_id(id)
        if chat is None:
            log.error(f"[CHAT-DB] Chat not found: {id}")
            return None
        ...
        # Ensure messages dict exists to prevent KeyError
        if "messages" not in history:
            history["messages"] = {}
        
        log.debug(f"[CHAT-DB] Message role: {message.get('role')}")
        log.debug(f"[CHAT-DB] Message content length: {len(message.get('content', ''))}")

        if message_id in history.get("messages", {}):
            log.debug(f"[CHAT-DB] Updating existing message")
            history["messages"][message_id] = {
                **history["messages"][message_id],
                **message,
            }
        else:
            log.debug(f"[CHAT-DB] Inserting new message")
        ...
        log.info(f"[CHAT-DB] Message upsert complete - currentId: {message_id}")
        return self.update_chat_by_id(id, chat)
