...
from open_webui.routers.video import (
    generate_video,
    GenerateVideoForm,
    save_generated_video_from_url,
    VIDEO_CONFIG,
)
...
async def extract_response_content(res):
    """Extract content from dict or JSONResponse"""
    if isinstance(res, JSONResponse):
        ...
        return await get_content_from_response(res)
...
async def chat_web_search_handler(
    ...
    try:
        ...
        response = await get_content_from_response(res)
...
async def chat_image_generation_handler(
    ...
    log.info("="*80)
    log.info("[IMAGE-GEN-HANDLER] 🎨 ENTRY - Starting image generation handler")
    ...
    log.info(f"[IMAGE-GEN-HANDLER] Chat ID: {chat_id}")
    log.info(f"[IMAGE-GEN-HANDLER] Message ID: {message_id}")
    log.info(f"[IMAGE-GEN-HANDLER] Event emitter present: {__event_emitter__ is not None}")
    
    if not chat_id or not isinstance(chat_id, str):
        log.warning("[IMAGE-GEN-HANDLER] ⚠️ Missing chat_id, aborting")
        return form_data
    
    # ✅ SIMPLIFIED: Always use form_data messages
    message_list = form_data.get("messages", [])
    log.info(f"[IMAGE-GEN-HANDLER] 📨 Using {len(message_list)} messages from form_data")
    
    user_message = get_last_user_message(message_list)
    log.info(f"[IMAGE-GEN-HANDLER] 💬 User message: {user_message[:100] if user_message else 'NONE'}...")
    
    if not user_message:
        log.error("[IMAGE-GEN-HANDLER] ❌ No user message found, cannot generate image")
        return form_data
    
    # Send status (optional, for UI feedback)
    if __event_emitter__:
        ...
        log.info("[IMAGE-GEN-HANDLER] 📤 Sent status: Creating image")
        
    prompt = user_message
    message_images = get_images_from_messages(message_list)
    log.info(f"[IMAGE-GEN-HANDLER] 🖼️ Found {len(message_images)} image sets in messages")
    ...
    log.info(f"[IMAGE-GEN-HANDLER] 📸 Processing {len(input_images)} input images")

    system_message_content = ""
    generated_files = []  # Track generated files

    # IMAGE EDIT vs IMAGE CREATE logic
    if len(input_images) > 0 and request.app.state.config.ENABLE_IMAGE_EDIT:
        log.info("[IMAGE-GEN-HANDLER] 🔄 IMAGE EDIT MODE - editing existing images")
        log.info(f"[IMAGE-GEN-HANDLER] Edit prompt: {prompt[:100]}...")
        ...
        try:
            ...
            log.info(f"[IMAGE-GEN-HANDLER] ✅ Generated {len(images)} edited images")
            ...
            # ✅ SHARE VIA METADATA (CRITICAL for main.py preservation)
            metadata["generated_files"] = generated_files
            log.info(f"[IMAGE-GEN-HANDLER] 🔗 Shared {len(generated_files)} files via metadata")

            # ✅ SAVE TO DATABASE IMMEDIATELY
            if chat_id and message_id and not chat_id.startswith("local:"):
                try:
                    print("saving in detabase immediately after generation.........")
                    ...
                    log.info(f"[IMAGE-GEN-HANDLER] 💾 Saved {len(generated_files)} files to database")
                except Exception as e:
                    log.error(f"[IMAGE-GEN-HANDLER] ❌ Failed to save files to database: {e}")

            # Emit events for real-time UI updates (optional)
            if __event_emitter__:
                await __event_emitter__(
                    {
                        "type": "status",
                ...
                log.info("[IMAGE-GEN-HANDLER] 📤 Sent status: Image created (edit)")
                ...
                log.info(f"[IMAGE-GEN-HANDLER] 📤 Sent {len(generated_files)} image files to client")
            ...
            log.info("[IMAGE-GEN-HANDLER] ✅ Set success system message")
            
        except Exception as e:
            log.error(f"[IMAGE-GEN-HANDLER] ❌ IMAGE EDIT ERROR: {e}", exc_info=True)
            ...
            if __event_emitter__:
                ...
                log.info("[IMAGE-GEN-HANDLER] 📤 Sent error status")
        ...
        # CREATE NEW IMAGE
        log.info("[IMAGE-GEN-HANDLER] ✨ IMAGE CREATE MODE - generating new image")
        
        # Prompt enhancement
        if request.app.state.config.ENABLE_IMAGE_PROMPT_GENERATION:
            log.info("[IMAGE-GEN-HANDLER] 🤖 Enhancing prompt with LLM")
            try:
                ...
                log.info("[IMAGE-GEN-HANDLER] ✅ Got prompt enhancement response")
                
                response = await extract_response_content(res)
                log.info(f"[IMAGE-GEN-HANDLER] 📝 Enhanced response: {str(response)[:200]}...")
                ...
                try:
                    ...
                    log.info(f"[IMAGE-GEN-HANDLER] ✅ Extracted enhanced prompt: {prompt[:100]}...")
                except Exception as e:
                    log.warning(f"[IMAGE-GEN-HANDLER] ⚠️ Failed to parse enhancement, using original: {e}")
                    prompt = user_message

            except Exception as e:
                log.error(f"[IMAGE-GEN-HANDLER] ❌ Prompt enhancement failed: {e}", exc_info=True)
                prompt = user_message
        else:
            log.info("[IMAGE-GEN-HANDLER] ⏭️ Skipping prompt enhancement (disabled)")

        log.info(f"[IMAGE-GEN-HANDLER] 🎯 Final prompt for generation: {prompt[:150]}...")
        
        try:
            log.info("[IMAGE-GEN-HANDLER] 🚀 Calling image_generations()")
            images = await image_generations(
             ...
             # ✅ FIX: Check if images is None
            if images is None:
                log.error("[IMAGE-GEN-HANDLER] ❌ image_generations() returned None")
                raise Exception("Image generation returned no results. Check your image generation configuration.")
            
            if not isinstance(images, list):
                log.error(f"[IMAGE-GEN-HANDLER] ❌ Unexpected return type: {type(images)}")
                raise Exception(f"Image generation returned unexpected type: {type(images)}")
            
            if len(images) == 0:
                log.warning("[IMAGE-GEN-HANDLER] ⚠️ Image generation returned empty list")
                raise Exception("Image generation returned no images")
            
            log.info(f"[IMAGE-GEN-HANDLER] ✅ Generated {len(images)} new images")
            log.info(f"[IMAGE-GEN-HANDLER] 📸 Image URLs: {[img.get('url', 'N/A')[:50] for img in images]}")
            ...
            # ✅ SAVE TO DATABASE IMMEDIATELY (DISABLED - handled in process_chat_response)
            # if chat_id and message_id and not chat_id.startswith("local:"):
            #     try:
            #         # print("entering in local to save in database")
            #         # Chats.upsert_message_to_chat_by_id_and_message_id(
            #         #     chat_id,
            #         #     message_id,
            #         #     {
            ...
            #         #     },
            #         # )
            #         log.info(f"[IMAGE-GEN-HANDLER] ⏭️ Skipped immediate save - deferring to process_chat_response")
            #     except Exception as e:
            #         log.error(f"[IMAGE-GEN-HANDLER] ❌ Failed to save files to database: {e}", exc_info=True)
            ...
            if __event_emitter__:
                ...
                log.info("[IMAGE-GEN-HANDLER] 📤 Sent status: Image created")
            ...
            log.error(f"[IMAGE-GEN-HANDLER] ❌ IMAGE CREATE ERROR: {e}", exc_info=True)
    ...
    if system_message_content:
        log.info("[IMAGE-GEN-HANDLER] 📝 Adding system message to form_data")
        form_data["messages"] = add_or_update_system_message(
            system_message_content, form_data["messages"]
        )
        log.info("[IMAGE-GEN-HANDLER] ✅ System message added")
    else:
        log.warning("[IMAGE-GEN-HANDLER] ⚠️ No system message content generated")

    log.info("[IMAGE-GEN-HANDLER] ✅ EXIT - Handler completed successfully")
    log.info("="*80)
    return form_data
...
# ============================================================
# middleware.py  —  PATCH for chat_video_generation_handler
# ============================================================
# Only this function needs changing. Everything else in
# middleware.py stays identical.
#
# KEY CHANGE: pass `request` into generate_video() so that
# _create_openai_video_generation can resolve the correct
# provider (base URL + API key) from request.app.state.OPENAI_MODELS
# instead of always defaulting to the first configured provider.
# ============================================================

async def chat_video_generation_handler(
    request: Request, form_data: dict, extra_params: dict, user
):
    log.info("entering into chat_video_generation_handler")
    metadata = extra_params.get("__metadata__", {})
    chat_id = metadata.get("chat_id", None)
    __event_emitter__ = extra_params.get("__event_emitter__", None)

    # CHANGED: removed `not __event_emitter__` from condition
    if not chat_id or not isinstance(chat_id, str):
        return form_data

    # ADDED: safe emit helper
    async def safe_emit(event: dict):
        if __event_emitter__:
    ...
    if chat_id.startswith("local:"):
        ...
        message_id = chat.chat.get("history", {}).get("currentId") if chat else None
        message_list = get_message_list(messages_map, message_id)

        # ── FALLBACK ──────────────────────────────────────────────────────────
        # For brand-new chats the DB history is still empty at this point.
        # form_data["messages"] always has the full in-flight message list,
        # so use it whenever the DB lookup comes back empty.
        if not message_list:
            log.info("[VIDEO-GEN] DB history empty — using form_data messages as fallback")
            message_list = form_data.get("messages", [])
        # ─────────────────────────────────────────────────────────────────────
    ...
    try:
        chat_selected_model = form_data.get("model", None)
        if VIDEO_CONFIG.VIDEO_GENERATION_ENGINE == "replicate":
            model_override = (
                chat_selected_model
                if isinstance(chat_selected_model, str) and ":" in chat_selected_model
                else None
            )
        else:
            model_override = (
                chat_selected_model if isinstance(chat_selected_model, str) else None
            )
        ...
        if result.get("status") == "succeeded" and result.get("video_url"):
            ...
            try:
                stored_filename = await save_generated_video_from_url(result["video_url"])
                persisted_video_url = (
                    f"{str(request.base_url).rstrip('/')}/api/v1/video/files/{stored_filename}"
                )
            except Exception:
                log.exception(
                    "Failed to persist generated video locally. Falling back to provider URL."
                )
            # ---- SAME PATTERN AS IMAGE HANDLER ----
            generated_files = [
                {
                    "type": "video",
                    "url": persisted_video_url,
                }
            ]
            metadata["generated_files"] = generated_files
            log.info(f"Shared video file via metadata: {persisted_video_url}")
            ...
            # CHANGED: include video URL in system message for non-WebSocket frontends
            system_message_content = (
                f"<context>The requested video has been created successfully. "
                f"Video URL: {persisted_video_url}. "
                f"Share this URL with the user so they can access their video.</context>"
            )
        else:
            error_message = result.get("error", "Unknown video generation error.")

            await safe_emit(
                {
                    "type": "status",
                    "data": {
                        "description": "An error occurred while generating a video",
                        "done": True,
                    },
                }
            )

            system_message_content = f"<context>Video generation was attempted but failed because of an error. The system is currently unable to generate the video. Tell the user that the following error occurred: {error_message}</context>"
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    # -> Chat Files
    # === IMAGE GENERATION TRACKING START ===
    log.info("="*80)
    log.info("[IMAGE-GEN-FLOW] 🎨 PROCESS_CHAT_PAYLOAD - ENTRY")
    log.info(f"[IMAGE-GEN-FLOW] User ID: {user.id}")
    log.info(f"[IMAGE-GEN-FLOW] Model: {form_data.get('model')}")
    
    # Check all possible locations for image_generation flag
    features = form_data.get("features", {})
    metadata_img_gen = metadata.get("image_generation", False)
    features_img_gen = features.get("image_generation", False)
    
    log.info(f"[IMAGE-GEN-FLOW] 📍 Flags detected:")
    log.info(f"[IMAGE-GEN-FLOW]   - metadata.image_generation: {metadata_img_gen}")
    log.info(f"[IMAGE-GEN-FLOW]   - features.image_generation: {features_img_gen}")
    log.info(f"[IMAGE-GEN-FLOW]   - form_data.metadata.image_generation: {form_data.get('metadata', {}).get('image_generation', False)}")
    log.info("="*80)
    ...
    # ✅ SET IMAGE GENERATION FLAG IMMEDIATELY
    if features.get("image_generation"):
        if "metadata" not in form_data:
            form_data["metadata"] = {}
        form_data["metadata"]["image_generation"] = True
    if features:
        log.info(f"[IMAGE-GEN-FLOW] 🔍 Processing features: {list(features.keys())}")
        ...
        if "image_generation" in features and features["image_generation"]:
            log.info("[IMAGE-GEN-FLOW] ✅ IMAGE GENERATION FEATURE DETECTED")
            log.info(f"[IMAGE-GEN-FLOW] 📝 Last user message: {get_last_user_message(form_data['messages'])[:100]}...")
            # Skip forced image generation when native FC is enabled - model can use generate_image tool
            if metadata.get("params", {}).get("function_calling") != "native":
                form_data = await chat_image_generation_handler(
                    request, form_data, extra_params, user
                )
            log.info("[IMAGE-GEN-FLOW] ✅ chat_image_generation_handler completed")
            # Add image_generation flag to metadata to signal SLM router to skip processing
            if "metadata" not in form_data:
                form_data["metadata"] = {}
            form_data["metadata"]["image_generation"] = True
        ...
        for tool_id in tool_ids:
            if tool_id.startswith("server:mcp:"):
                try:
                    ...
                    log.error(f"[IMAGE-GEN-FLOW] ❌ ERROR in handler: {e}", exc_info=True)
                    log.debug(e)
                    if event_emitter:
                        await event_emitter(
                            {
                    ...
                    if TASKS.TAGS_GENERATION in tasks and tasks[TASKS.TAGS_GENERATION]:
                        ...
                        if res and isinstance(res, dict):
                            ...
                            try:
                                ...
                                # Preserve existing files when updating tags in message metadata if applied to first message
                                # (Note: update_chat_tags_by_id updates chat record, not message record usually, 
                                # but we ensure no collisions if we were to update message metadata here)
        ...
        if event_emitter:
            try:
                if isinstance(response, dict) or isinstance(response, JSONResponse):
                    ...
                    if choices and choices[0].get("message", {}).get("content"):
                        ...
                        if content:
                            ...
                            if metadata.get("generated_files"):
                                message_data["files"] = metadata["generated_files"]
                    ...
                    # ✅ Inject generated files from metadata into response
                    if metadata.get("generated_files"):
                        response_data["files"] = metadata["generated_files"]
                        log.info(f"[process_chat_response] 🔗 Injected {len(metadata['generated_files'])} generated files into response")
                                ...
                                if "selected_model_id" in data:
                                    ...
                                    if value:
                                        ...
                                        if ENABLE_REALTIME_CHAT_SAVE:
                                            # Save message in the database
                                            # Preserve existing files to prevent overwriting
                                            existing_msg = Chats.get_message_by_id_and_message_id(
                                                metadata["chat_id"], metadata["message_id"]
                                            )
                                            existing_files = existing_msg.get("files", []) if existing_msg else []
                                            ...
                                            Chats.upsert_message_to_chat_by_id_and_message_id(
                                                ...
                                                metadata["message_id"],
                                                {
                                                    ...
                                                    "files": existing_files,  # Preserve files
                                                },
                                            )
                                        else:
                                            data = {
