...
async def send_post_request(
    ...
    log.info(f"[OLLAMA-SEND] send_post_request - ENTRY")
    log.debug(f"[OLLAMA-SEND] URL: {url}")
    log.debug(f"[OLLAMA-SEND] Stream: {stream}")
    log.debug(f"[OLLAMA-SEND] Has key: {key is not None}")
    ...
    try:
        ...
        if isinstance(payload_json, dict):
            sanitized = {
                ...
                'stream': payload_json.get('stream'),
            }
            log.debug(f"[OLLAMA-SEND] Payload: {json.dumps(sanitized, default=str)}")
    except Exception:
        pass
    ...
    try:
        ...
        log.debug("[OLLAMA-SEND] Created aiohttp session")
        ...
        log.debug(f"[OLLAMA-SEND] Sending POST request to: {url}")
        ...
        log.debug(f"[OLLAMA-SEND] Response status: {r.status}")

        if r.ok is False:
            log.error(f"[OLLAMA-SEND] Request failed with status: {r.status}")
            try:
                res = await r.json()
                await cleanup_response(r, session)
                if "error" in res:
                    log.error(f"[OLLAMA-SEND] API Error: {res['error']}")
                    raise HTTPException(status_code=r.status, detail=res["error"])
            except HTTPException as e:
                raise e
            except Exception as e:
                log.error(f"[OLLAMA-SEND] Failed to parse error response: {e}")
                raise HTTPException(
        ...
        if stream:
            ...
            log.info(f"[OLLAMA-SEND] Returning StreamingResponse with status: {r.status}")
            ...
            res = await r.json()
            log.info(f"[OLLAMA-SEND] Returning JSON response")
            return res
        ...
        detail = f"Ollama: {e}"
        log.exception(f"[OLLAMA-SEND] Exception: {detail}")
...
async def generate_openai_chat_completion(
    ...
    log.info("[OLLAMA-ROUTER] generate_openai_chat_completion - ENTRY")
    log.debug(f"[OLLAMA-ROUTER] User ID: {user.id if hasattr(user, 'id') else 'unknown'}")
    ...
    log.debug(f"[OLLAMA-ROUTER] Incoming form_data: {json.dumps(sanitized_payload, default=str)}")

    metadata = form_data.pop("metadata", None)
    log.debug(f"[OLLAMA-ROUTER] Metadata: {json.dumps(metadata, default=str) if metadata else None}")
    ...
    log.info(f"[OLLAMA-ROUTER] Final model_id: {model_id}")

    model_info = Models.get_model_by_id(model_id, db=db)
    log.debug(f"[OLLAMA-ROUTER] Model info found: {model_info is not None}")
    
    if model_info:
        if model_info.base_model_id:
            log.info(f"[OLLAMA-ROUTER] Using base_model_id: {model_info.base_model_id}")
            payload["model"] = model_info.base_model_id

        params = model_info.params.model_dump()
        log.debug(f"[OLLAMA-ROUTER] Model params: {json.dumps(params, default=str)[:500]}")

        if params:
            system = params.pop("system", None)
            log.debug(f"[OLLAMA-ROUTER] System prompt: {system[:100] if system else None}")
        ...
        if user.role == "user":
            if not (
                ...
                log.error(f"[OLLAMA-ROUTER] Access denied for user {user.id}")
                raise HTTPException(
                    status_code=403,
                    detail="Model not found",
                )
        ...
        if user.role != "admin":
            log.error(f"[OLLAMA-ROUTER] Non-admin user accessing unknown model")
            raise HTTPException(
                status_code=403,
                detail="Model not found",
            )
    ...
    url, url_idx = await get_ollama_url(request, payload["model"], url_idx)
    log.info(f"[OLLAMA-ROUTER] Ollama URL: {url}, url_idx: {url_idx}")
    ...
    log.debug(f"[OLLAMA-ROUTER] API config: {json.dumps({k:v for k,v in api_config.items() if k != 'api_key'}, default=str)}")

    prefix_id = api_config.get("prefix_id", None)
    if prefix_id:
        log.info(f"[OLLAMA-ROUTER] Removing prefix: {prefix_id}")
        payload["model"] = payload["model"].replace(f"{prefix_id}.", "")

    log.info(f"[OLLAMA-ROUTER] Sending request to: {url}/v1/chat/completions")
    log.debug(f"[OLLAMA-ROUTER] Payload: {json.dumps(payload, default=str)[:500]}")
