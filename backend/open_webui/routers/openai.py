...
async def generate_chat_completion(
    ...
    log.info("[OPENAI-ROUTER] generate_chat_completion - ENTRY")
    log.debug(f"[OPENAI-ROUTER] User ID: {user.id if hasattr(user, 'id') else 'unknown'}")
    log.debug(f"[OPENAI-ROUTER] User role: {user.role if hasattr(user, 'role') else 'unknown'}")
    ...
    log.debug(f"[OPENAI-ROUTER] Incoming form_data: {json.dumps(sanitized_payload, default=str)}")
    
    if BYPASS_MODEL_ACCESS_CONTROL:
        log.debug("[OPENAI-ROUTER] Bypassing model access control")
        bypass_filter = True
    ...
    log.debug(f"[OPENAI-ROUTER] Metadata: {json.dumps(metadata, default=str) if metadata else None}")

    model_id = form_data.get("model")
    log.debug(f"[OPENAI-ROUTER] Looking up model_id: {model_id}")
    model_info = Models.get_model_by_id(model_id, db=db)
    log.debug(f"[OPENAI-ROUTER] Model info found: {model_info is not None}")

    # Check model info and override the payload
    if model_info:
        log.debug(f"[OPENAI-ROUTER] Model has custom config")
        if model_info.base_model_id:
            ...
            log.info(f"[OPENAI-ROUTER] Using base_model_id: {base_model_id}")
            payload["model"] = base_model_id
            model_id = base_model_id

        params = model_info.params.model_dump()
        params = model_info.params.model_dump()
        log.debug(f"[OPENAI-ROUTER] Model params: {json.dumps(params, default=str)[:500]}")

        if params:
            system = params.pop("system", None)
            log.debug(f"[OPENAI-ROUTER] System prompt: {system[:100] if system else None}")
        ...
        if not bypass_filter and user.role == "user":
            if not (
                ...
                log.error(f"[OPENAI-ROUTER] Access denied for user {user.id}")
                raise HTTPException(
                    status_code=403,
        ...
        if user.role != "admin":
            log.error(f"[OPENAI-ROUTER] Non-admin user accessing unknown model")
            raise HTTPException(
                status_code=403,
                detail="Model not found",
            )
    ...
    log.debug(f"[OPENAI-ROUTER] Fetching OPENAI_MODELS for: {model_id}")
    model = request.app.state.OPENAI_MODELS.get(model_id)
    if model:
        idx = model["urlIdx"]
        log.info(f"[OPENAI-ROUTER] Found model at URL index: {idx}")
    else:
        log.error(f"[OPENAI-ROUTER] Model not found in OPENAI_MODELS: {model_id}")
        raise HTTPException(
            status_code=404,
            detail="Model not found",
        )
    ...
    log.debug(f"[OPENAI-ROUTER] API config: {json.dumps({k:v for k,v in api_config.items() if k != 'api_key'}, default=str)}")

    prefix_id = api_config.get("prefix_id", None)
    if prefix_id:
        log.info(f"[OPENAI-ROUTER] Removing prefix: {prefix_id}")
        payload["model"] = payload["model"].replace(f"{prefix_id}.", "")

    # Add user info to the payload if the model is a pipeline
    if "pipeline" in model and model.get("pipeline"):
        log.debug("[OPENAI-ROUTER] Adding user info to pipeline payload")
        payload["user"] = {
            "name": user.name,
            "id": user.id,
            "email": user.email,
