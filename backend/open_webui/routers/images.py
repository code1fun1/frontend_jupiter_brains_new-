...
async def image_generations(
    ...
    log.info("="*80)
    log.info("[IMAGE-GENERATIONS] 🎨 ENTRY")
    log.info(f"[IMAGE-GENERATIONS] Engine: {request.app.state.config.IMAGE_GENERATION_ENGINE}")
    log.info(f"[IMAGE-GENERATIONS] Model: {request.app.state.config.IMAGE_GENERATION_MODEL}")
    log.info(f"[IMAGE-GENERATIONS] Enabled: {request.app.state.config.ENABLE_IMAGE_GENERATION}")
    log.info(f"[IMAGE-GENERATIONS] Prompt: {form_data.prompt[:100]}...")
    log.info(f"[IMAGE-GENERATIONS] Size: {form_data.size or request.app.state.config.IMAGE_SIZE}")
    log.info(f"[IMAGE-GENERATIONS] Steps: {form_data.steps or request.app.state.config.IMAGE_STEPS}")

       # Check if image generation is enabled
    if not request.app.state.config.ENABLE_IMAGE_GENERATION:
        log.error("[IMAGE-GENERATIONS] ❌ Image generation is DISABLED in config")
        raise HTTPException(
            status_code=400,
    ...
    log.info(f"[IMAGE-GENERATIONS] Using engine: {engine}")
