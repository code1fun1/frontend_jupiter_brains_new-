...
async def analyze_query_for_model_selection(
    query: str,
    request: Request,
    user=Depends(get_verified_user),
):
    """
    Analyze a query and return which model would be auto-selected.
    Useful for UI preview and debugging.
    """

    # 🔴 ENSURE MODELS ARE LOADED (CRITICAL)
    if not request.app.state.MODELS:
        await get_all_models(request, user=user)

    runtime_models = list(request.app.state.MODELS.values())

    if not runtime_models:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No runtime models available",
        )

    # ✅ Build selector-compatible model list
    available_models = [
        {
            "name": m["id"],
            "type": "on_prem" if m.get("connection_type") == "local" else "cloud",
        }
        ...
        if (
            ...
            and "/" not in m["id"]
            and m.get("pipeline", {}).get("type") != "filter"
        )
    ]



    analysis = auto_select_model(
        query=query,
