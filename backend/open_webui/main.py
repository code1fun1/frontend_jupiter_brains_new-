...
async def lifespan(app: FastAPI):
    ...
    # Configure video generation (Replicate) on startup
    if ENABLE_VIDEO_GENERATION:
        from open_webui.routers.video import set_video_config

        # Get values from PersistentConfig objects
        replicate_key = REPLICATE_API_KEY.value if hasattr(REPLICATE_API_KEY, 'value') else REPLICATE_API_KEY
        enabled = ENABLE_VIDEO_GENERATION.value if hasattr(ENABLE_VIDEO_GENERATION, 'value') else ENABLE_VIDEO_GENERATION
        video_model = VIDEO_GENERATION_MODEL.value if hasattr(VIDEO_GENERATION_MODEL, 'value') else VIDEO_GENERATION_MODEL
        ...
        log.info("✅ Video generation enabled")
...
app.state.config.REPLICATE_API_KEY = REPLICATE_API_KEY
...
async def chat_completion(
    ...
    print("Chat completion request received with form_data:", form_data)
    if not request.app.state.MODELS:
        await get_all_models(request, user=user)
    ...
    try:
        ...
        user_metadata = form_data.get("metadata", {})
        ...
        # ✅ STEP 3: Merge user metadata (user values overwrite server values)
        metadata.update(user_metadata)
        
        
        log.info(f"[DEBUG] Final merged metadata keys: {list(metadata.keys())}")
        if "slm_decision" in user_metadata:
            log.info(f"[DEBUG] User sent slm_decision={user_metadata['slm_decision']}")
        ...
        log.debug(f"[process_chat] Error processing chat metadata: {e}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e),
        )
    ...
    async def process_chat(request, form_data, user, metadata, model):
        try:
            # Track request details before processing
            log.info(
                f"[process_chat] Processing chat request - "
                f"model: {model}, "
                f"metadata: {metadata}, "
                f"form_data keys: {list(form_data.keys()) if hasattr(form_data, 'keys') else 'N/A'}"
            )
            log.debug(
                f"[process_chat] Full request details - "
                f"request: {request}, "
                f"user: {user}, "
            ...
            # process_chat_payload → chat_video_generation_handler already ran the
            # video generation and stored the URL(s) in metadata["generated_files"].
            # The selected model (e.g. minimax/video-01) is *not* an LLM, so calling
            # chat_completion_handler on it would hit /chat/completions on the video
            # provider and get a 404.  We bypass that call entirely and return a
            # synthetic (empty) completion so the caller receives the files.
            if metadata.get("generated_files"):
                log.info("[VIDEO-GEN] Skipping LLM call — video already generated.")
                return {
                    "id": "video-" + str(uuid4()),
            ...
            log.debug(f"[process_chat] Error processing chat payload: {e}")
            if metadata.get("chat_id") and metadata.get("message_id"):
                ...
                try:
                    ...
                    pass
            # Return a proper error response instead of None (which becomes null)
            return JSONResponse(
                status_code=500,
    ...
    if (
        ...
        # ✅ No recommendation (or SLM disabled/already processed) — normal flow unchanged
        task_id, task = await create_task(
            request.app.state.redis,
            process_chat(request, form_data, user, metadata, model),
            id=metadata["chat_id"],
        )
        # await the SAME task, don't call process_chat again
        response = await task  

        # Extract JSON body from the Response object so we return clean JSON
        response_data = {}
        if hasattr(response, "body"):
            try:
                response_data = json.loads(response.body)
            except Exception:
                response_data = {"raw": response.body.decode("utf-8", errors="replace")}
        elif isinstance(response, dict):
            response_data = response
        
        generated_files = metadata.get("generated_files", [])

        return {**response_data, "status": True, "task_id": task_id,  **({"files": generated_files} if generated_files else {})}
    else:
        # ---- CHANGED: also check for generated_files in else branch ----
        response = await process_chat(request, form_data, user, metadata, model)
        
        generated_files = metadata.get("generated_files", [])
        
        if generated_files:
            # response is either a Response object or dict
            if hasattr(response, "body"):
                try:
                    response_data = json.loads(response.body)
                except Exception:
                ...
                response_data = {}
            return {**response_data, "files": generated_files}
            ...
            **(
                {
                    ...
                    "enable_video_generation": video.VIDEO_CONFIG.ENABLE_VIDEO_GENERATION,
