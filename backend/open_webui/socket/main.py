...
def log_socket_event(event_type, details=None):
    """Helper to log socket events"""
    log.debug(f"[SOCKET-EVT] {event_type}")
    if details:
        log.debug(f"[SOCKET-EVT] Details: {json.dumps(details, default=str)}")
...
def get_event_emitter(request_info, update_db=True):
    async def __event_emitter__(event_data):
        ...
        log.debug(f"[SOCKET-EVT] user_id: {user_id}")
        log.debug(f"[SOCKET-EVT] chat_id: {chat_id}")
        log.debug(f"[SOCKET-EVT] message_id: {message_id}")
        log.debug(f"[SOCKET-EVT] event_type: {event_data.get('type')}")
        
        # Log event data based on type
        if event_data.get('type') == 'message':
            content = event_data.get('data', {}).get('content', '')
            log.debug(f"[SOCKET-EVT] message content length: {len(content)}")
        elif event_data.get('type') == 'status':
            status_data = event_data.get('data', {})
            log.debug(f"[SOCKET-EVT] status data: {json.dumps(status_data, default=str)}")
        elif event_data.get('type') == 'done':
            log.info("[SOCKET-EVT] Stream completed (done event)")
        ...
        log.debug(f"[SOCKET-EVT] Emitted event to user:{user_id}")
        ...
        if (
            ...
            log.debug(f"[SOCKET-EVT] Updating database for message")

            if "type" in event_data and event_data["type"] == "status":
                log.debug(f"[SOCKET-EVT] Adding status to message")
            ...
            if "type" in event_data and event_data["type"] == "message":
                log.debug(f"[SOCKET-EVT] Appending message content")
                ...
                if message:
                    ...
                    new_content = event_data.get("data", {}).get("content", "")
                    log.debug(f"[SOCKET-EVT] Appending content: {len(new_content)} chars")
                    content += new_content
            ...
            if "type" in event_data and event_data["type"] == "replace":
                log.debug(f"[SOCKET-EVT] Replacing message content")
                content = event_data.get("data", {}).get("content", "")
                log.debug(f"[SOCKET-EVT] Replace content length: {len(content)}")
            ...
            if "type" in event_data and event_data["type"] == "embeds":
                log.debug(f"[SOCKET-EVT] Adding embeds to message")
                message = Chats.get_message_by_id_and_message_id(
                    request_info["chat_id"],
                    request_info["message_id"],
                )
            ...
            if "type" in event_data and event_data["type"] == "files":
                log.debug(f"[SOCKET-EVT] Adding files to message")
                message = Chats.get_message_by_id_and_message_id(
                    request_info["chat_id"],
                    request_info["message_id"],
                )
            ...
            if event_data.get("type") in ["source", "citation"]:
                log.debug(f"[SOCKET-EVT] Adding source/citation to message")
            ...
            log.debug(f"[SOCKET-EVT] Skipping DB update (update_db={update_db}, message_id={message_id})")
    ...
    if (
        ...
        log.debug(f"[SOCKET-EVT] Returning event emitter for chat:{request_info['chat_id']}")
        return __event_emitter__
    else:
        log.warning(f"[SOCKET-EVT] Missing required fields in request_info")
        return None
